...
func calculate_hexagon(side_length):
	var vertices = []
	for i in range(6):
		var angle = deg_to_rad(30 + 60 * i)
		var x = side_length * cos(angle)
		var y = side_length * sin(angle)
		vertices.append(Vector2(x, y))
	return vertices

func _on_area2d_input_event(viewport, event, shape_idx):
		if event is InputEventMouseButton and event.pressed and event.button_index == MOUSE_BUTTON_LEFT:
			if parent.tile_dictionary[tile_key]["In_sight"]:
...
func _on_area_2d_2_area_entered(area):
	if area.is_in_group("Unit"):
		...
		parent.update_fog("Tile_" + str(id), parent.get_node("Unit").previous_tile_id, unit_node)
